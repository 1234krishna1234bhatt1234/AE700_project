%% ========================================================================
%% q4.m  —  Linear Design Models (Q4 of AE700 project)
%% ========================================================================
%% Computes trim, transfer functions, and state-space models.
%%
%% Run this script from the mavsim_simulink/ folder.
%% ========================================================================
clear; clc; close all;

%% ---- Resolve paths robustly (works regardless of current directory) ----
base_dir = fileparts(mfilename('fullpath'));   % folder containing q4.m
...
%% ========================================================================
%% STEP (iii)-(iv): Compute TRIM for various gamma (wings-level flight)
%% ========================================================================

Va_des  = 200;                                    % desired airspeed (m/s)
gammas  = [-10, -5, 0, 5, 10] * pi/180;          % path angles to test
R_des   = Inf;                                    % wings-level

fprintf('\n--- Trim Results for Wings-Level Flight (Va=%.0f m/s) ---\n', Va_des);
fprintf('%8s %10s %10s %10s %10s %10s\n', 'gamma_deg', 'alpha_deg', 'theta_deg', 'delta_e', 'delta_t', 'climb_m_s');
...
fprintf('\nVerification: only altitude changes with gamma. Va stays at %.0f m/s.\n', Va_des);

%% ---- Step (v): Trim for constant turn (load factor n=1.2) ------------

n_load = 1.2;
phi_trim_turn = acos(1/n_load);
R_turn = Va_des^2 / (P.gravity * tan(phi_trim_turn));

fprintf('\n--- Trim for Constant Turn (n=%.1f) ---\n', n_load);
...
fprintf('  alpha=%.2f deg, delta_e=%.4f, delta_t=%.4f\n', ...
    yt_turn(1)*180/pi, ut_turn(1), ut_turn(4));

%% ========================================================================
%% STEP (vi): Transfer Functions (B&M Ch. 5.4)
%% ========================================================================

% Use wings-level trim (gamma=0)
idx0       = find(gammas == 0);
...
figure('Name','Q4(vi): Transfer Function Step Responses','NumberTitle','off');
...
%% ========================================================================
%% STEP (vii): State-Space Models (B&M Eq. 5.43 & 5.50)
%% ========================================================================
...
% Longitudinal SS: states = [u, w, q, theta, h]
A_lon = [ X_u,  X_w,  X_q,  -g*cos(theta_trim), 0;
          Z_u,  Z_w,  Z_q,  -g*sin(theta_trim), 0;
          M_u,  M_w,  M_q,   0,                 0;
          0,    0,    1,     0,                 0;
          sin(theta_trim), -cos(theta_trim), 0, Va_trim, 0 ];
...
L_v  = P.C_ell_beta *qbar*P.S_wing*P.b/(P.Jx*Va_trim);
...
% Lateral SS: states = [v, p, r, phi, psi]
A_lat = [ Y_v,  Y_p,  -(Va_trim-Y_r),  g*cos(theta_trim), 0;
          L_v,  L_p,   L_r,             0,                 0;
          N_v,  N_p,   N_r,             0,                 0;
...
fprintf('\n--- Longitudinal A_lon (states: u,w,q,theta,h) ---\n');
disp(A_lon);
fprintf('--- B_lon (inputs: delta_e, delta_t) ---\n');
disp(B_lon);
fprintf('\n--- Lateral A_lat (states: v,p,r,phi,psi) ---\n');
disp(A_lat);
...
sys_lon = ss(A_lon, B_lon, eye(5), zeros(5,2));
sys_lat = ss(A_lat, B_lat, eye(5), zeros(5,2));

figure('Name','Q4(vii): State-Space Bode','NumberTitle','off');
set(gcf,'Position',[100 50 900 400]);
subplot(1,2,1)
bode(sys_lon(3,1)); title('Longitudinal: q/\delta_e','FontSize',12); grid on;
subplot(1,2,2)
bode(sys_lat(2,1)); title('Lateral: p/\delta_a','FontSize',12); grid on;
sgtitle('Q4(vii): State-Space Bode Verification','FontSize',13,'FontWeight','bold');

fprintf('\n=== Q4 Complete ===\n');

%% ========================================================================
%% LOCAL FUNCTION: Analytic Trim
%% ========================================================================
